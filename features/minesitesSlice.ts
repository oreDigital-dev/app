...
const mineSitesSlice = createSlice({
    ...
    reducers: {
        initializeMinesites: (state: RootState, action: PayloadAction<{ minesites: Array<any> }>) => {
            ...
            console.log(action.payload.minesites)
            console.log(Object.keys(action.payload.minesites))
        ...
        createMineSite: (state: RootState, payload: PayloadAction<{ name: string, minerals: string, province: string, district: string, sector: string, cell: string, village: string }>) => {
...
export const { initializeMinesites, setSelectedMineSite, createMineSite } = mineSitesSlice.actions;
