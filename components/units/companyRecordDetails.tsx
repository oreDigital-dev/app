  ...
  return (
    <div className='flex items-center mb-4 p-3 min-w-[250px] gap-4 border cursor-pointer rounded-lg shadow-sm'>
        <div className='w-full'>
          ...
          {props.reports == 0? (<h1 className='font-bold text-2xl' style={{
            color: COLORS[Math.floor(Math.random() * COLORS.length)]
          }}>{props.sites} Sites</h1>) : <h1 className='font-bold text-xl' style={{
            color: COLORS[Math.floor(Math.random() * COLORS.length)]}}>{props.reports} Reports</h1>}
